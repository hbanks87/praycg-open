#!/usr/bin/env python3
"""
PRAYCG MediaPrep GUI v1.7A - ALS Fullscreen Start-Pulse + AnchorPrep Patch
...
Given one master .mp4, this tool creates:
  ...
  6. DRAFT predeclared anchor JSON/CSV + lock checklist
...
v1.7A patch focus:
  ...
  - Generates DRAFT predeclared anchor files; exact rendered times must be filled
    and finalized into *_LOCKED.json before the runner can marker-register them.
...
SCHEMA = "PRAYCG_media_prep_suite_v1_8_anchorprep_cet_eet_als_fullscreen_start_pulse"
CUE_SCHEMA = "PRAYCG_number_cue_schedule_v1_7A"
...
def make_qc_checklists(out_dir: Path, manifest: Dict[str, Any]) -> None:
    ...
    (out_dir / "CONTROL_AUDIO_QC_CHECKLIST_v1_7A.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.7A Control Audio QC Checklist
    ...
    """), encoding="utf-8")
    (out_dir / "VISUAL_SOURCE_STAMP_QC_CHECKLIST_v1_7A.md").write_text(textwrap_dedent(f"""
    ...
    (out_dir / "ALS_PT19_SENSOR_TIMING_QC_CHECKLIST_v1_7A.md").write_text(textwrap_dedent(f"""
...
def _anchor_preset_rows(preset: str) -> List[Dict[str, Any]]:
    """Return editable draft anchor rows. Times are intentionally blank until human review."""
    preset = (preset or "generic").strip().lower()
    common = {
        "source": "media_structural_human_review_required",
        "claim_level": "DRAFT_NOT_CONFIRMATORY_UNTIL_LOCKED_AND_RUNNER_REGISTERED",
        "condition_scope": "all_branches",
        "time_basis": "rendered_video_sec",
        "pre_window_sec": [-15.0, 0.0],
        "expected_peak_search_window_sec": [0.0, 10.0],
        "theta_carryover_window_sec": [10.0, 30.0],
        "comparison_rule": "Target must exceed matched Control and Contextual Override under artifact/timing veto for endpoint interpretation.",
        "needs_exact_rendered_timecode": True,
        "rendered_time_sec": None,
        "content_time_sec": None,
    }
    def row(anchor_id: str, description: str, **kw: Any) -> Dict[str, Any]:
        r = dict(common)
        r.update({"anchor_id": anchor_id, "description": description})
        r.update(kw)
        return r
    generic = [
        row("ANCHOR_A1_PRIMARY_MEANING_EVENT", "Primary predeclared media-structural meaning event. Fill exact rendered_time_sec after reviewing the final MP4."),
        row("ANCHOR_A2_SECONDARY_MEANING_EVENT", "Secondary predeclared meaning event or recognition point. Fill exact rendered_time_sec."),
        row("ANCHOR_A3_TERMINAL_TAPER_RELEASE", "Terminal taper/release event. Fill exact rendered_time_sec; keep enough post-event content for theta carryover."),
    ]
    contact = [
        row("CONTACT_A1_PENSACOLA_REALIZATION", "Ellie recognizes the surreal beach as a memory-derived Pensacola-like environment.", special_controls=["visual_luminance_penalty_required", "visual_control_channels_required"]),
        row("CONTACT_A2_AVATAR_FATHER_RECOGNITION", "The figure resolves into the father/avatar; semantic contradiction and attachment recognition.", predicted_mred_quadrant="MR_HIGH_ENC_HIGH", expected_peak_search_window_sec=[0.0, 12.0]),
        row("CONTACT_A3_EXISTENTIAL_PAYLOAD_ONSET", "Onset of the central philosophical payload about loneliness, dreams/nightmares, and connection.", predicted_signal_shape="sustained_meaninggamma_tsp_elevation", expected_peak_search_window_sec=[0.0, 35.0], theta_carryover_window_sec=[35.0, 65.0]),
        row("CONTACT_A4_SMALL_MOVES_TAPER_RELEASE", "Terminal taper/release after the final payload line.", expected_peak_search_window_sec=[-10.0, 5.0], theta_carryover_window_sec=[0.0, 30.0], allow_paired_washout_continuation=True),
    ]
    eternal = [
        row("ES_A1_BEACH_HOUSE_COLLAPSE_ONSET", "First clear physical onset of beach-house/memory-space destabilization.", special_controls=["visual_motion_penalty_required", "visual_luminance_penalty_required"]),
        row("ES_A2_MEET_ME_IN_MONTAUK_LINE", "Onset of the 'Meet me in Montauk' future-directed semantic command.", predicted_mred_quadrant="MR_HIGH_ENC_HIGH", expected_peak_search_window_sec=[0.0, 8.0], theta_carryover_window_sec=[8.0, 30.0]),
        row("ES_A3_POST_MONTAUK_RELEASE_TAPER", "Release/taper moment after the Montauk line and memory-collapse climax.", expected_peak_search_window_sec=[-10.0, 5.0], theta_carryover_window_sec=[0.0, 30.0], allow_paired_washout_continuation=True),
    ]
    if preset in {"contact", "contact_beach"}:
        return contact
    if preset in {"eternal_sunshine", "eternal", "montauk"}:
        return eternal
    if preset in {"contact_eternal", "contact_eternal_sunshine", "both"}:
        return contact + eternal
    return generic


def _write_anchor_csv(path: Path, anchors: List[Dict[str, Any]]) -> None:
    fields: List[str] = []
    for row in anchors:
        for k in row.keys():
            if k not in fields:
...
def make_anchor_prep_files(out_dir: Path, project: str, manifest: Dict[str, Any], preset: str = "generic") -> Dict[str, Any]:
    """Create DRAFT predeclared-anchor files beside cue_schedule. Human must fill times and finalize."""
    anchors = _anchor_preset_rows(preset)
    prefix = f"predeclared_anchors_{project}_v1_7A_DRAFT"
    out_json = out_dir / f"{prefix}.json"
    out_csv = out_dir / f"{prefix}.csv"
    checklist = out_dir / f"ANCHOR_LOCK_CHECKLIST_{project}_v1_7A.md"
    payload = {
        "schema": "PRAYCG_predeclared_anchor_schedule_v1_7A",
        "status": "DRAFT_DO_NOT_USE_AS_CONFIRMATORY_UNTIL_LOCKED",
        "created_utc": now_utc_iso(),
        "project_name": project,
        "anchor_preset": preset,
        "media_prep_manifest": str(out_dir / "media_prep_manifest_v1_8.json"),
        "cue_schedule_json": manifest.get("outputs", {}).get("cue_schedule_json"),
        "target_video": manifest.get("outputs", {}).get("target_cued"),
        "override_video": manifest.get("outputs", {}).get("override_cued"),
        "control_video": manifest.get("outputs", {}).get("control_phase_scrambled"),
        "target_video_sha256": manifest.get("sha256", {}).get("target_cued"),
        "override_video_sha256": manifest.get("sha256", {}).get("override_cued"),
        "control_video_sha256": manifest.get("sha256", {}).get("control_phase_scrambled"),
        "cue_schedule_sha256": sha256_file(Path(manifest.get("outputs", {}).get("cue_schedule_json"))) if manifest.get("outputs", {}).get("cue_schedule_json") else None,
        "content_start_offset_sec": manifest.get("sensor_timing_design", {}).get("content_start_offset_sec", 0.0),
        "analysis_exclusion_prefix_sec": manifest.get("sensor_timing_design", {}).get("analysis_exclusion_prefix_sec", 0.0),
        "timecode_rule": "All rendered_time_sec values must be measured on the final generated Target/Override MP4, after ALS prefix/cue rendering. Do not use source-master time unless converted with content_start_offset_sec.",
        "lock_instruction": "Review final Target MP4, fill exact rendered_time_sec for each anchor, then run scripts/praycg_anchor_lock_finalizer_v1_7A.py to create a *_LOCKED.json file for the runner.",
        "anchors": anchors,
    }
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    _write_anchor_csv(out_csv, anchors)
    checklist.write_text(f"""# PRAYCG v1.7A Anchor Lock Checklist — {project}
...
## Files generated
- Draft JSON: `{out_json.name}`
- Draft CSV: `{out_csv.name}`
...
```bat
python scripts\\praycg_anchor_lock_finalizer_v1_7A.py --draft "{out_json}"
```
...
""", encoding="utf-8")
    return {"anchor_draft_json": str(out_json), "anchor_draft_csv": str(out_csv), "anchor_lock_checklist": str(checklist), "anchor_preset": preset}

def make_report(out_dir: Path, manifest: Dict[str, Any]) -> None:
    lines = []
    lines.append("# PRAYCG MediaPrep Suite v1.8 ALS Fullscreen Start-Pulse + AnchorPrep + StimulusFingerprint/CET/EET Patch Report")
    lines.append("")
    lines.append(f"Created UTC: `{manifest.get('created_utc')}`")
    lines.append(f"Project: `{manifest.get('project_name')}`")
    lines.append("")
    lines.append("## Outputs")
    for key in ["master_input", "cleaned_master", "target_cued", "override_cued", "control_phase_scrambled", "cue_schedule_json", "cue_schedule_csv", "anchor_draft_json", "anchor_draft_csv", "anchor_lock_checklist"]:
    ...
    (out_dir / "PRAYCG_MediaPrep_v1_7A_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_media_prep(args: argparse.Namespace, progress_cb=None) -> Path:
    ...
    out_dir = out_root / f"{project}_MediaPrep_v1_7A_{stamp}"
    ...
    if args.clean_visual_mask:
        cleaned_master = out_dir / f"stimulus_master_cleaned_{project}_v1_7A.mp4"
        if progress_cb:
    ...
    target_cued = out_dir / f"stimulus_target_cued_{project}_v1_7A.mp4"
    override_cued = out_dir / f"stimulus_override_cued_{project}_v1_7A.mp4"
    control = out_dir / f"stimulus_control_cued_phase_scrambled_{project}_v1_7A.mp4"
    schedule_json = out_dir / f"cue_schedule_{project}_v1_7A.json"
    schedule_csv = out_dir / f"cue_schedule_{project}_v1_7A.csv"
    ...
    target_pre_sensor = out_dir / f"_working_target_number_cued_no_sensor_{project}_v1_7A.mp4"
    control_pre_sensor = out_dir / f"_working_control_phase_scrambled_no_sensor_{project}_v1_7A.mp4"
    ...
    anchor_prep_summary = None
    if bool(getattr(args, "make_anchor_template", True)):
        anchor_prep_summary = make_anchor_prep_files(out_dir, project, manifest, preset=str(getattr(args, "anchor_preset", "generic")))
        manifest["anchor_prep"] = anchor_prep_summary
        manifest["outputs"].update(anchor_prep_summary)

    (out_dir / "media_prep_manifest_v1_8.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    ...
    if args.run_fingerprint:
        ...
        script_path = Path(__file__).resolve().parent / "praycg_stimulus_fingerprint_cet_eet_v1_8.py"
        if script_path.exists() and control.exists():
            if progress_cb:
                progress_cb("Running StimulusFingerprint/CET/EET v1.8 on generated suite...")
            fp_out = out_dir / "qc"
            fp_out.mkdir(parents=True, exist_ok=True)
            cmd = [sys.executable, str(script_path), "--project-name", project, "--control", str(control), "--target", str(target_cued), "--override", str(override_cued), "--cue-schedule-json", str(schedule_json), "--out-root", str(fp_out), "--video-sample-hz", str(args.fingerprint_sample_fps), "--resize-width", str(args.fingerprint_resize_width), "--merge-hz", "4", "--flat-output", "--overwrite"]
            ...
            if proc.returncode != 0:
                ...
                if progress_cb: progress_cb(f"StimulusFingerprint/CET/EET complete: {fp_out}")
        else:
            if progress_cb:
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PRAYCG MediaPrep GUI v1.8 ALS Fullscreen Start-Pulse + AnchorPrep + StimulusFingerprint/CET/EET Patch")
    ...
    p.add_argument("--make-anchor-template", action="store_true", default=True, help="Generate DRAFT predeclared-anchor JSON/CSV beside the cue schedule")
    p.add_argument("--no-anchor-template", action="store_false", dest="make_anchor_template", help="Do not generate anchor-prep draft files")
    p.add_argument("--anchor-preset", default="generic", choices=["generic", "contact", "eternal_sunshine", "contact_eternal_sunshine"], help="DRAFT anchor scaffold preset to write beside generated media")
...
def launch_gui(defaults: argparse.Namespace) -> None:
    ...
    v1.7A patch: the Run button now gives immediate visible feedback, validates
    ...
    root = tk.Tk()
    root.title("PRAYCG MediaPrep Suite v1.8 - ALS Fullscreen Start-Pulse + AnchorPrep")
    ...
    header = ttk.Label(main, text="PRAYCG MediaPrep Suite v1.8", font=("Segoe UI", 15, "bold"))
    ...
    bools["make_anchor_template"] = tk.BooleanVar(value=True)
    ttk.Checkbutton(main, text="Generate DRAFT predeclared anchor schedule next to cue_schedule", variable=bools["make_anchor_template"]).grid(row=r, column=1, columnspan=2, sticky="w", padx=5, pady=2); r += 1
    add_row("Anchor preset (generic/contact/eternal_sunshine/contact_eternal_sunshine)", "anchor_preset", "generic", row=r); r += 1
    ...
    def write_crash_file(ns: Optional[argparse.Namespace], tb: str) -> Optional[Path]:
        try:
            ...
            out_root.mkdir(parents=True, exist_ok=True)
            crash = out_root / f"PRAYCG_MediaPrep_v1_7A_CRASH_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    run_btn = ttk.Button(btn_frame, text="Run Stimulus Media Suite v1.8", command=on_run)
