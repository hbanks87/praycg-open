#!/usr/bin/env python3
"""
PRAYCG MediaPrep GUI v1.6Q - GUI Button Hotfix
...
v1.6Q patch focus:
  - Keeps v1.6P visual source-stamp and stronger control-audio logic.
  - Fixes the GUI run-button failure mode with visible logging, validation,
    thread-safe status updates, and crash logs.
...
SCHEMA = "PRAYCG_media_prep_suite_v1_6Q_gui_button_hotfix"
CUE_SCHEMA = "PRAYCG_number_cue_schedule_v1_6Q"
...
def render_cued_video(master_mp4: Path, output_cued_mp4: Path, events: List[CueEvent], position: str, badge_alpha: float, font_scale_factor: float, overwrite: bool, progress_cb=None, max_seconds: Optional[float] = None) -> Dict[str, Any]:
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Could not open {master_mp4}")
    tmp_dir = Path(tempfile.mkdtemp(prefix="praycg_v16Q_cued_"))
...
def make_qc_checklists(out_dir: Path, manifest: Dict[str, Any]) -> None:
    ...
    (out_dir / "CONTROL_AUDIO_QC_CHECKLIST_v1_6Q.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.6Q Control Audio QC Checklist
    ...
    """), encoding="utf-8")
    (out_dir / "VISUAL_SOURCE_STAMP_QC_CHECKLIST_v1_6Q.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.6Q Visual Source-Stamp / Watermark QC Checklist
...
def make_report(out_dir: Path, manifest: Dict[str, Any]) -> None:
    lines = []
    lines.append("# PRAYCG MediaPrep Suite v1.6Q Control-Validity Patch Report")
    lines.append("")
    ...
    (out_dir / "PRAYCG_MediaPrep_v1_6Q_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_media_prep(args: argparse.Namespace, progress_cb=None) -> Path:
    ...
    out_dir = out_root / f"{project}_MediaPrep_v1_6Q_{stamp}"
    ...
    if args.clean_visual_mask:
        cleaned_master = out_dir / f"stimulus_master_cleaned_{project}_v1_6Q.mp4"
        if progress_cb:
    ...
    target_cued = out_dir / f"stimulus_target_cued_{project}_v1_6Q.mp4"
    override_cued = out_dir / f"stimulus_override_cued_{project}_v1_6Q.mp4"
    control = out_dir / f"stimulus_control_cued_phase_scrambled_{project}_v1_6Q.mp4"
    schedule_json = out_dir / f"cue_schedule_{project}_v1_6Q.json"
    schedule_csv = out_dir / f"cue_schedule_{project}_v1_6Q.csv"

    if progress_cb:
        progress_cb("Rendering cue-embedded Target video...")
    cue_render_summary = render_cued_video(working_master, target_cued, events, position=args.position, badge_alpha=float(args.badge_alpha), font_scale_factor=float(args.font_scale_factor), overwrite=bool(args.overwrite), progress_cb=progress_cb, max_seconds=args.max_seconds)
    ...
    (out_dir / "media_prep_manifest_v1_6Q.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PRAYCG MediaPrep GUI v1.6Q Control-Validity Patch")
...
def launch_gui(defaults: argparse.Namespace) -> None:
    ...
    v1.6Q patch: the Run button now gives immediate visible feedback, validates
    ...
    root = tk.Tk()
    root.title("PRAYCG MediaPrep Suite v1.6Q - GUI Button Hotfix")
    ...
    header = ttk.Label(main, text="PRAYCG MediaPrep Suite v1.6Q", font=("Segoe UI", 15, "bold"))
    header.grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 4))
    sub = ttk.Label(main, text="One master MP4 -> cleaned/cued Target + identical Override + phase-scrambled Control + cue schedule + QC files")
    ...
    def write_crash_file(ns: Optional[argparse.Namespace], tb: str) -> Optional[Path]:
        try:
            ...
            out_root.mkdir(parents=True, exist_ok=True)
            crash = out_root / f"PRAYCG_MediaPrep_v1_6Q_CRASH_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    run_btn = ttk.Button(btn_frame, text="Run Stimulus Media Suite v1.6Q", command=on_run)
    ...
    ttk.Label(main, text="Workflow: optional master cleanup -> cued Target + identical Override -> phase-scrambled Control with stronger audio control.", anchor="w").grid(row=r, column=0, columnspan=4, sticky="w")
