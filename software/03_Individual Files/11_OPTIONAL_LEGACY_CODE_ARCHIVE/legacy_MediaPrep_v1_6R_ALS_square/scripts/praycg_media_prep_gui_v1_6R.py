#!/usr/bin/env python3
"""
PRAYCG MediaPrep GUI v1.6R - ALS-PT19 Timing-Pulse Patch
...
v1.6R patch focus:
  ...
  - Adds an optional ALS-PT19 / photodiode-style timing square rendered into all
    branch videos for physical display-onset validation.
  - Default timing pulse is video-start only to avoid introducing a periodic
    visual entrainment confound. Optional cue-start pulses are available for
    dedicated hardware-validation runs.
...
SCHEMA = "PRAYCG_media_prep_suite_v1_6R_als_pt19_timing_pulse"
CUE_SCHEMA = "PRAYCG_number_cue_schedule_v1_6R"
...
def render_cued_video(master_mp4: Path, output_cued_mp4: Path, events: List[CueEvent], position: str, badge_alpha: float, font_scale_factor: float, overwrite: bool, progress_cb=None, max_seconds: Optional[float] = None) -> Dict[str, Any]:
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Could not open {master_mp4}")
    tmp_dir = Path(tempfile.mkdtemp(prefix="praycg_v16R_cued_"))
...
def make_sensor_pulse_schedule(
    ...
    if mode not in {"video_start", "cue_start", "both", "none"}:
        raise ValueError("sensor_pulse_mode must be one of: video_start, cue_start, both, none")
    if mode == "none":
        return []
    pulses: List[SensorPulseEvent] = []
    idx = 1
    if mode in {"video_start", "both"}:
        dur = max(0.05, float(video_start_duration_sec))
        pulses.append(SensorPulseEvent(idx, "video_start", 0.0, min(dur, duration_sec), min(dur, duration_sec), position))
        idx += 1
...
def render_sensor_timing_video(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="praycg_v16R_sensor_"))
...
def make_qc_checklists(out_dir: Path, manifest: Dict[str, Any]) -> None:
    ...
    (out_dir / "CONTROL_AUDIO_QC_CHECKLIST_v1_6R.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.6R Control Audio QC Checklist
    ...
    """), encoding="utf-8")
    (out_dir / "VISUAL_SOURCE_STAMP_QC_CHECKLIST_v1_6R.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.6R Visual Source-Stamp / Watermark QC Checklist
    ...
    (out_dir / "ALS_PT19_SENSOR_TIMING_QC_CHECKLIST_v1_6R.md").write_text(textwrap_dedent(f"""
    # PRAYCG v1.6R ALS-PT19 / Photodiode Timing QC Checklist

    Project: `{manifest.get('project_name')}`
    Sensor timing enabled: `{sensor_design.get('enabled')}`
    Sensor square position: `{sensor_design.get('position')}`
...
def make_report(out_dir: Path, manifest: Dict[str, Any]) -> None:
    lines = []
    lines.append("# PRAYCG MediaPrep Suite v1.6R ALS-PT19 Timing-Pulse Patch Report")
    lines.append("")
    ...
    for k, v in ca.items():
        lines.append(f"- **{k}:** `{v}`")
    lines.append("")
    lines.append("## ALS-PT19 / photodiode timing square")
    ...
    lines.append("5. The ALS timing square is a physical display-timing channel. It should be taped over by the light sensor and should not be interpreted as biological photonic data.")
    lines.append("")
    ...
    (out_dir / "PRAYCG_MediaPrep_v1_6R_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_media_prep(args: argparse.Namespace, progress_cb=None) -> Path:
    ...
    out_dir = out_root / f"{project}_MediaPrep_v1_6R_{stamp}"
    ...
    if args.clean_visual_mask:
        cleaned_master = out_dir / f"stimulus_master_cleaned_{project}_v1_6R.mp4"
        if progress_cb:
    ...
    target_cued = out_dir / f"stimulus_target_cued_{project}_v1_6R.mp4"
    override_cued = out_dir / f"stimulus_override_cued_{project}_v1_6R.mp4"
    control = out_dir / f"stimulus_control_cued_phase_scrambled_{project}_v1_6R.mp4"
    schedule_json = out_dir / f"cue_schedule_{project}_v1_6R.json"
    schedule_csv = out_dir / f"cue_schedule_{project}_v1_6R.csv"
    ...
    sensor_pulses = make_sensor_pulse_schedule(
        ...
        mode=str(getattr(args, "sensor_pulse_mode", "video_start")),
        position=str(getattr(args, "sensor_position", "lower_right")),
        video_start_duration_sec=float(getattr(args, "sensor_video_start_duration", 1.0)),
    ...
    target_pre_sensor = out_dir / f"_working_target_number_cued_no_sensor_{project}_v1_6R.mp4"
    control_pre_sensor = out_dir / f"_working_control_phase_scrambled_no_sensor_{project}_v1_6R.mp4"
    ...
    metadata = {
        ...
        "sensor_timing_design": {
            ...
            "position": str(getattr(args, "sensor_position", "lower_right")),
            "pulse_mode": str(getattr(args, "sensor_pulse_mode", "video_start")),
            "video_start_duration_sec": float(getattr(args, "sensor_video_start_duration", 1.0)),
            "cue_pulse_duration_sec": float(getattr(args, "sensor_cue_pulse_duration", 0.25)),
    ...
    (out_dir / "media_prep_manifest_v1_6R.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PRAYCG MediaPrep GUI v1.6R ALS-PT19 Timing-Pulse Patch")
    ...
    p.add_argument("--sensor-pulse-mode", default="video_start", choices=["video_start", "cue_start", "both", "none"], help="Timing-square pulse mode. Default video_start minimizes periodic visual perturbation.")
    ...
    p.add_argument("--sensor-video-start-duration", type=float, default=1.0, help="Seconds of white timing square at the start of each video")
...
def launch_gui(defaults: argparse.Namespace) -> None:
    ...
    v1.6R patch: the Run button now gives immediate visible feedback, validates
    ...
    root = tk.Tk()
    root.title("PRAYCG MediaPrep Suite v1.6R - ALS-PT19 Timing-Pulse Patch")
    ...
    header = ttk.Label(main, text="PRAYCG MediaPrep Suite v1.6R", font=("Segoe UI", 15, "bold"))
    header.grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 4))
    sub = ttk.Label(main, text="One master MP4 -> cleaned/cued Target + identical Override + phase-scrambled Control + ALS timing square + QC files")
    ...
    add_row("ALS pulse mode", "sensor_pulse_mode", "video_start", row=r); r += 1
    add_row("ALS video-start pulse seconds", "sensor_video_start_duration", "1.0", row=r); r += 1
    ...
    def write_crash_file(ns: Optional[argparse.Namespace], tb: str) -> Optional[Path]:
        try:
            ...
            out_root.mkdir(parents=True, exist_ok=True)
            crash = out_root / f"PRAYCG_MediaPrep_v1_6R_CRASH_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    run_btn = ttk.Button(btn_frame, text="Run Stimulus Media Suite v1.6R", command=on_run)
    ...
    ttk.Label(main, text="Workflow: optional cleanup -> number-cued Target/Override -> phase-scrambled Control -> optional ALS timing square on all branches.", anchor="w").grid(row=r, column=0, columnspan=4, sticky="w")
