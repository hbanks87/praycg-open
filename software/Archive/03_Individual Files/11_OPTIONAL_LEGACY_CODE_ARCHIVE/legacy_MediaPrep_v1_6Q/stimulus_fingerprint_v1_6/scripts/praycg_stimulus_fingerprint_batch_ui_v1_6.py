#!/usr/bin/env python3
"""
PRAYCG StimulusFingerprint Suite v1.6
...
def make_master_report(out_dir: Path, cfg: SuiteConfig, condition_rows: List[Dict[str, Any]], comparison_rows: List[Dict[str, Any]], cue_meta: Dict[str, Any]) -> None:
    ...
    lines: List[str] = []
    lines += [
        "# PRAYCG StimulusFingerprint Suite v1.6 Master Report",
        "",
    ...
    (report_dir / "PRAYCG_StimulusFingerprint_v1_6_master_report.md").write_text("\n".join(lines), encoding="utf-8")


def run_suite(cfg: SuiteConfig, log=print) -> Path:
    validate_config(cfg)
    project = sanitize_name(cfg.project_name)
    if getattr(cfg, "flat_output", False):
        out_dir = cfg.out_root.resolve()
        if out_dir.exists() and getattr(cfg, "overwrite", False):
            shutil.rmtree(out_dir)
    else:
        out_dir = cfg.out_root.resolve() / f"{project}_SF_v1_6_{now_stamp()}"
    ...
    manifest = {
        "schema": "PRAYCG_stimulus_fingerprint_suite_v1_6",
    ...
    master_summary = {
        "schema": "PRAYCG_stimulus_fingerprint_suite_summary_v1_6",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "project_name": cfg.project_name,
    ...
    log("\nDONE. Master report:")
    log(str(master / "PRAYCG_StimulusFingerprint_v1_6_master_report.md"))
    return out_dir
...
def launch_gui() -> None:
    ...
    root = tk.Tk()
    root.title("PRAYCG StimulusFingerprint Suite v1.6")
    ...
    vars: Dict[str, Any] = {
        ...
        "flat_output": tk.BooleanVar(value=False),
    }
    ...
    title = ttk.Label(main, text="PRAYCG StimulusFingerprint Suite v1.6", font=("Segoe UI", 16, "bold"))
    ...
    ttk.Checkbutton(opts, text="Apply cue-visibility QC to Control too", variable=vars["apply_cue_to_control"]).pack(side="left", padx=(0, 18))
    ttk.Checkbutton(opts, text="Short/flat output folder", variable=vars["flat_output"]).pack(side="left")
    ...
    def build_config_from_ui() -> SuiteConfig:
        ...
        return SuiteConfig(
            ...
            flat_output=bool(vars["flat_output"].get()),
            overwrite=True,
        )
    ...
    def run_clicked() -> None:
        ...
        log_text.insert("end", "Starting PRAYCG StimulusFingerprint Suite v1.6...\n")
        log_text.see("end")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PRAYCG StimulusFingerprint Suite v1.6 UI/batch runner.")
    ...
    p.add_argument("--flat-output", action="store_true", help="Write directly into --out-root instead of adding another timestamped nesting folder. Recommended when called from MediaPrep.")
    p.add_argument("--overwrite", action="store_true", help="If --flat-output is set and --out-root exists, remove and recreate it.")
    return p.parse_args()
