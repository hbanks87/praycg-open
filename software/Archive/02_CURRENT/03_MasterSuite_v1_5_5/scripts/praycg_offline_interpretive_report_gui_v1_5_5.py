#!/usr/bin/env python3
"""Small Tkinter GUI wrapper for the PRAYCG offline interpretive report generator v1.5.5."""
...
SCRIPT = Path(__file__).with_name("praycg_offline_interpretive_report_generator_v1_5_5.py")

class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PRAYCG Offline Interpretive Report v1.5.5")
    ...
    def _build(self):
        ...
        ttk.Label(pad, text="PRAYCG Offline Interpretive Report Generator v1.5.5", font=("Arial", 15, "bold")).pack(anchor="w")
        ttk.Label(pad, text="Reads a Master Comprehensive output folder and writes a rule-based Markdown/TXT explanation. No internet or AI model required.", wraplength=760).pack(anchor="w", pady=(4, 10))
        self._row(pad, "Analysis folder", self.analysis)
        self._row(pad, "Output folder", self.outdir)
        fr = ttk.Frame(pad); fr.pack(fill="x", pady=5)
        ttk.Label(fr, text="Run label", width=22).pack(side="left")
