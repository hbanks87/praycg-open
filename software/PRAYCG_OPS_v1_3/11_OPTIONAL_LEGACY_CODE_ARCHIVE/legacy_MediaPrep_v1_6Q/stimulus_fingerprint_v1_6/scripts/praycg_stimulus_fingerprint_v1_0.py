...
def integrate_trapezoid(y, x=None, dx=1.0, axis=-1):
    ...
    np.trapz was removed/deprecated in newer NumPy builds; np.trapezoid is
    the supported replacement. The fallback keeps compatibility with older
    NumPy environments.
    """
...
def band_power(freqs: np.ndarray, psd: np.ndarray, low: float, high: float) -> float:
    ...
    if not np.any(mask):
        return 0.0
    return float(integrate_trapezoid(psd[mask], freqs[mask]))
