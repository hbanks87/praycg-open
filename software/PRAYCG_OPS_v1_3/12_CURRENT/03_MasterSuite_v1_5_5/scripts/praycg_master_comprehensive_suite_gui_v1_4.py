...
class MasterSuiteGUI:
    ...
    def _build(self):
        ...
        recommended = ttk.LabelFrame(tab_modules, text="Recommended confirmatory compression — A-MRED pathway")
        recommended.pack(fill="x", pady=(0, 10))
        ttk.Label(recommended, text=(
            "BOXED PRIMARY PATH: Timing/QC + StimulusFingerprint/CET-R + artifact/confound gates "
            "→ Anchor-Locked MRED (A-MRED).  Secondary summaries: NIP/CII/IAQ and TTI.  "
            "All remaining modules are retained but should be labeled exploratory/convergence/diagnostic."
        ), wraplength=900, foreground="#003366").pack(anchor="w", padx=8, pady=6)
        ttk.Label(recommended, text=(
            "For a strict A-MRED endpoint, load a frame-verified *_LOCKED.json anchor file before acquisition; "
            "estimated/draft anchors are pilot-only."
