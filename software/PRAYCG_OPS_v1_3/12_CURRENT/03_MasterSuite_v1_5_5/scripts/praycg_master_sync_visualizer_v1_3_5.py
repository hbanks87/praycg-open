#!/usr/bin/env python3
"""PRAYCG MasterSync Visualizer v1.3.5.
...
EVENT_BGR = {
    ...
    "artifact": (50,50,180), "als": (255,255,255), "nip": (90,255,220), "bit": (40,255,210), "cii": (80,210,255), "iaq": (60,180,255), "cet": (220,220,80), "eet": (200,160,255), "cet_eet": (210,190,255), "immersion": (90,255,220), "mred_itp": (255,210,90), "mred_itp_candidate": (255,235,60), "acg_complexity": (180,255,60), "ocu_blink_release": (255,160,90), "itp": (255,210,90), "amred": (255,120,220), "amred_primary": (255,80,220), "mred_peak": (255,100,180), "mred_resolution": (120,255,180), "primary_endpoint": (255,120,220), "other": (160,160,160)
}
PUBLIC_CATS = {"annotation","candidate_kht","kht_topo","mred","tti","tti_positive","topo_osm","g2theta","postpeak_pncc","nip","bit","cii","iaq","cet","eet","cet_eet","immersion","mred_itp","mred_itp_candidate","acg_complexity","ocu_blink_release","itp","amred","amred_primary","mred_peak","mred_resolution","primary_endpoint","protocol"}
    ...
    if args.analysis_out:
        ...
        try:
            ...
            for name in [
                ...
                "cet_tracking_summary.csv", "eet_endogenous_echo_tracking.csv", "mred_itp_visual_overlay.csv", "acg_event_table.csv", "ocu_event_table.csv", "mred_itp_anchor_summary.csv", "amred_visual_overlay.csv", "amred_anchor_endpoint_table.csv", "amred_primary_endpoint_summary.csv", "nupi_visual_overlay.csv", "nupi_anchor_polarity_table.csv", "nupi_run_summary.csv", "mred_peak_resolution_visual_overlay.csv", "mred_peak_resolution_anchor_table.csv", "mred_peak_resolution_run_summary.csv"
            ]:
                p = tables / name
...
def infer_cat(row,label,source):
    ...
    if "mred_peak" in text or "peak_resolution" in text and "peak" in text: return "mred_peak"
    if "mred_resolution" in text or "resolution_candidate" in text: return "mred_resolution"
...
def run_pipeline(args):
    ...
    out=Path(args.out or "PRAYCG_MasterSync_v1_3_5_Render.mp4").resolve(); out_dir=Path(args.out_dir).resolve() if args.out_dir else out.parent; out_dir.mkdir(parents=True,exist_ok=True)
...
def build_arg_parser():
    p=argparse.ArgumentParser(description="PRAYCG MasterSync Visualizer v1.3.5")
...
def launch_gui():
    ...
    class App(tk.Tk):
        def __init__(self):
            super().__init__(); self.title("PRAYCG MasterSync Visualizer v1.3.5"); self.geometry("860x840")
        ...
        def build(self):
            pad=ttk.Frame(self,padding=10); pad.pack(fill="both",expand=True); ttk.Label(pad,text="PRAYCG MasterSync Visualizer v1.3.5",font=("Arial",16,"bold")).pack(anchor="w")
