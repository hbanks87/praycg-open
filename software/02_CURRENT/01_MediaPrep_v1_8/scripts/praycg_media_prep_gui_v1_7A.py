...
SCHEMA = "PRAYCG_media_prep_suite_v1_7A_anchorprep_als_fullscreen_start_pulse"
...
def make_anchor_prep_files(out_dir: Path, project: str, manifest: Dict[str, Any], preset: str = "generic") -> Dict[str, Any]:
    ...
    payload = {
        ...
        "media_prep_manifest": str(out_dir / "media_prep_manifest_v1_7A.json"),
...
def make_report(out_dir: Path, manifest: Dict[str, Any]) -> None:
    lines = []
    lines.append("# PRAYCG MediaPrep Suite v1.7A ALS Fullscreen Start-Pulse + AnchorPrep Patch Report")
    lines.append("")
...
def run_media_prep(args: argparse.Namespace, progress_cb=None) -> Path:
    ...
    (out_dir / "media_prep_manifest_v1_7A.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PRAYCG MediaPrep GUI v1.7A ALS Fullscreen Start-Pulse + AnchorPrep Patch")
...
def launch_gui(defaults: argparse.Namespace) -> None:
    ...
    root = tk.Tk()
    root.title("PRAYCG MediaPrep Suite v1.7A - ALS Fullscreen Start-Pulse + AnchorPrep")
    ...
    header = ttk.Label(main, text="PRAYCG MediaPrep Suite v1.7A", font=("Segoe UI", 15, "bold"))
    ...
    run_btn = ttk.Button(btn_frame, text="Run Stimulus Media Suite v1.7A", command=on_run)
