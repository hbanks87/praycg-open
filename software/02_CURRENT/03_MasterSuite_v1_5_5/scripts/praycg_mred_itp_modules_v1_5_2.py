...
def apply_corrected_time_axis(ts: np.ndarray, data: np.ndarray, stream_name: str='obci_eeg1', inventory_csv: Optional[Path]=None) -> Tuple[np.ndarray, float, Dict[str, Any]]:
    inv_path = inventory_csv or Path('/mnt/data/PRAYCG_Contact_Run1_Analysis_v1_0/tables/stream_inventory_corrected.csv')
    info = {'alignment_method':'raw_xdf_timestamps'}
    if inv_path.exists():
        inv = pd.read_csv(inv_path)
        eeg = inv[inv['name'] == stream_name]
        if not eeg.empty:
            r = eeg.iloc[0]
...
def run_analysis(output_dir: Path, xdf: Path, event_path: Path, feature_path: Path, annotation_path: Path, mred_path: Path, stream_inventory_csv: Optional[Path]=None) -> Dict[str,Any]:
    ...
    ts, fs, alignment_info = apply_corrected_time_axis(ts_raw, data, stream_name=str(header.get('name','obci_eeg1')), inventory_csv=stream_inventory_csv)
...
def run_contact(output_dir: Path) -> Dict[str,Any]:
    return run_analysis(
        ...
        stream_inventory_csv=Path('/mnt/data/PRAYCG_Contact_Run1_Analysis_v1_0/tables/stream_inventory_corrected.csv')
    )
...
if __name__ == '__main__':
    import argparse
    ap=argparse.ArgumentParser(description='PRAYCG MRED-ITP v1.5.2: Algorithmic Complexity Gate and Ocular-Cognitive Unloading')
    ap.add_argument('--out-dir', required=True, help='Output analysis folder')
    ap.add_argument('--xdf', help='Input XDF')
    ap.add_argument('--event-log', help='PRAYCG event log JSON/CSV')
    ap.add_argument('--feature-csv', help='Master Suite feature table CSV')
    ap.add_argument('--annotation-csv', help='Annotation/anchor window CSV')
    ap.add_argument('--mred-event-csv', help='CandidateLocal KHT-topo/MRED event table CSV')
    ap.add_argument('--stream-inventory-csv', help='Optional corrected stream inventory with corrected_t_min/corrected_t_max')
    ap.add_argument('--contact-defaults', action='store_true', help='Use bundled Contact Run 1 paths')
    args=ap.parse_args()
    out=Path(args.out_dir)
    if out.exists(): shutil.rmtree(out)
    if args.contact_defaults:
        interp=run_contact(out)
    else:
        required=[args.xdf,args.event_log,args.feature_csv,args.annotation_csv,args.mred_event_csv]
        if any(v is None for v in required):
            raise SystemExit('Either pass --contact-defaults or provide --xdf, --event-log, --feature-csv, --annotation-csv, and --mred-event-csv')
        interp=run_analysis(
            out,
            Path(args.xdf),
            Path(args.event_log),
            Path(args.feature_csv),
            Path(args.annotation_csv),
            Path(args.mred_event_csv),
            Path(args.stream_inventory_csv) if args.stream_inventory_csv else None,
        )
