#!/usr/bin/env python3
"""
PRAYCG Unified Master Analysis + Visualizer v1.4.8
==================================================
...
VIS_SCRIPT = ROOT / "praycg_master_sync_visualizer_v1_3_5.py"
CONFOUND_RSM_SCRIPT = ROOT / "praycg_confound_rsm_modules_v1_4_6.py"
TOPO_OSM_SCRIPT = ROOT / "praycg_topo_osm_network_state_module_v1_4_7.py"
LSO_SCRIPT = ROOT / "praycg_lso_subtitle_override_v1_4_7.py"
TTI_SCRIPT = ROOT / "praycg_tti_reception_extraction_module_v1_4_8.py"
AMRED_SCRIPT = ROOT / "praycg_amred_endpoint_module_v1_5_3.py"
NUPI_SCRIPT = ROOT / "praycg_nupi_module_v1_5_4.py"
...
def build_visual_cmd(args: argparse.Namespace, analysis_folder: Path) -> List[str]:
    ...
    add("--render-mode", getattr(args, "visual_render_mode", "public_tsc"))
...
def run_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if getattr(args, "run_confound_rsm_modules", False):
        confound_cmd = [sys.executable, str(CONFOUND_RSM_SCRIPT), "--analysis-folder", str(analysis_folder)]
        if args.event_log:
            confound_cmd.extend(["--event-log", args.event_log])
        if args.cue_schedule_json:
            confound_cmd.extend(["--cue-schedule-json", args.cue_schedule_json])
        if args.cue_schedule_csv:
            confound_cmd.extend(["--cue-schedule-csv", args.cue_schedule_csv])
        if getattr(args, "ocm_cue_epoch_csv", ""):
            confound_cmd.extend(["--ocm-cue-epoch-csv", args.ocm_cue_epoch_csv])
        if getattr(args, "manual_confound_json", ""):
            confound_cmd.extend(["--manual-confound-json", args.manual_confound_json])
        confound_proc = run_cmd(confound_cmd)
        print(confound_proc.stdout)
        if confound_proc.returncode != 0:
            print(confound_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"Confound/RSM modules failed with code {confound_proc.returncode}")

    if getattr(args, "run_topo_osm_modules", False):
        topo_cmd = [sys.executable, str(TOPO_OSM_SCRIPT), "--analysis-folder", str(analysis_folder)]
        if getattr(args, "topo_feature_csv", ""):
            topo_cmd.extend(["--feature-csv", args.topo_feature_csv])
        topo_proc = run_cmd(topo_cmd)
        print(topo_proc.stdout)
        if topo_proc.returncode != 0:
            print(topo_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"Topo-OSM modules failed with code {topo_proc.returncode}")

    if getattr(args, "run_lso_modules", False):
        lso_cmd = [sys.executable, str(LSO_SCRIPT), "--analysis-folder", str(analysis_folder)]
        if getattr(args, "subtitle_file", ""):
            lso_cmd.extend(["--subtitle-file", args.subtitle_file])
        lso_proc = run_cmd(lso_cmd)
        print(lso_proc.stdout)
        if lso_proc.returncode != 0:
            print(lso_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"LSO/SPM modules failed with code {lso_proc.returncode}")

    if getattr(args, "run_tti_modules", False):
        tti_cmd = [sys.executable, str(TTI_SCRIPT), "--analysis-folder", str(analysis_folder)]
        if getattr(args, "tti_feature_csv", ""):
            tti_cmd.extend(["--feature-csv", args.tti_feature_csv])
        if getattr(args, "tti_event_table", ""):
            tti_cmd.extend(["--event-table", args.tti_event_table])
        if getattr(args, "ocm_cue_epoch_csv", ""):
            tti_cmd.extend(["--ocm-cue-epoch-csv", args.ocm_cue_epoch_csv])
        tti_proc = run_cmd(tti_cmd)
        print(tti_proc.stdout)
        if tti_proc.returncode != 0:
            print(tti_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"TTI modules failed with code {tti_proc.returncode}")


    if getattr(args, "run_amred_modules", False):
        amred_cmd = [sys.executable, str(AMRED_SCRIPT), "--analysis-folder", str(analysis_folder)]
        if getattr(args, "predeclared_anchor_file", ""):
            amred_cmd.extend(["--anchor-file", args.predeclared_anchor_file])
        if getattr(args, "amred_feature_csv", ""):
            amred_cmd.extend(["--feature-csv", args.amred_feature_csv])
        if getattr(args, "amred_event_table", ""):
            amred_cmd.extend(["--event-table", args.amred_event_table])
        amred_proc = run_cmd(amred_cmd)
        print(amred_proc.stdout)
        if amred_proc.returncode != 0:
            print(amred_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"A-MRED module failed with code {amred_proc.returncode}")

    if getattr(args, "run_nupi_modules", False):
        nupi_profile = getattr(args, "nupi_profile", "custom") or "custom"
        nupi_cmd = [sys.executable, str(NUPI_SCRIPT), "--analysis-folder", str(analysis_folder), "--profile", nupi_profile, "--run-name", args.project_name]
        nupi_proc = run_cmd(nupi_cmd)
        print(nupi_proc.stdout)
        if nupi_proc.returncode != 0:
            print(nupi_proc.stderr, file=sys.stderr)
            raise RuntimeError(f"NUPI module failed with code {nupi_proc.returncode}")

    result: Dict[str, Any] = {
        "schema": "PRAYCG_Unified_Master_Analysis_Visualizer_v1_5_3_run_result",
        "version": VERSION,
        "mode": args.mode,
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PRAYCG Unified Master Analysis + Visualizer v1.4.8")
    ...
    p.add_argument("--run-confound-rsm-modules", action="store_true", help="Run v1.4.6 AVSync/ExternalNoise/RSM/CVB/Squint modules after master analysis.")
    p.add_argument("--run-topo-osm-modules", action="store_true", help="Run v1.4.7 Topo-OSM network-state module after master analysis.")
    p.add_argument("--run-lso-modules", action="store_true", help="Run v1.4.7 Lexical/Subtitle Override module after master analysis.")
    p.add_argument("--run-tti-modules", action="store_true", help="Run v1.4.8 Thermodynamic Theft Index / Reception-Extraction Tradeoff module after master analysis.")
    p.add_argument("--run-amred-modules", action="store_true", help="Run v1.5.3 Anchor-Locked MRED primary endpoint module after master analysis.")
    p.add_argument("--run-nupi-modules", action="store_true", help="Run v1.5.4 Narrative Update Polarity Index after master analysis.")
    p.add_argument("--nupi-profile", default="custom", help="NUPI table profile: arrival, contact, field, or custom.")
    p.add_argument("--amred-feature-csv", default="", help="Optional feature CSV override for A-MRED module.")
    p.add_argument("--amred-event-table", default="", help="Optional CandidateLocal/MRED event table override for A-MRED module.")
    p.add_argument("--tti-feature-csv", default="", help="Optional feature CSV override for TTI module.")
    p.add_argument("--tti-event-table", default="", help="Optional CandidateLocal/MRED event table override for TTI module.")
    p.add_argument("--subtitle-file", default="", help="SRT/CSV subtitle schedule for LSO/SPM module.")
    p.add_argument("--topo-feature-csv", default="", help="Optional feature CSV override for Topo-OSM module.")
    p.add_argument("--ocm-cue-epoch-csv", default="", help="Optional OCM025 cue-epoch CSV for RSM/CVB/Squint modules.")
    p.add_argument("--manual-confound-json", default="", help="Optional manual retrospective confound addendum JSON for pre-PRAYCG1.9 runs.")
    ...
    p.add_argument("--visual-render-mode", default="public_tsc", choices=["public_tsc", "lab"], help="Visualizer render preset: public_tsc or lab.")
...
def launch_gui() -> int:
    ...
    class App(tk.Tk):
        def __init__(self):
            super().__init__()
            self.title("PRAYCG Unified Master Suite v1.5.3")
            ...
            self.vars = {k: tk.StringVar(value=v) for k, v in {
                ...
                "visual_video": "", "visual_condition": "target", "visual_events": "", "visual_feature_csv": "", "visual_out": "", "visual_render_mode": "public_tsc",
            }.items()}
            self.build()
        ...
        def build(self):
            ...
            ttk.Label(run, text="PRAYCG Unified Master Suite v1.5.3", font=("Arial", 16, "bold")).pack(anchor="w")
            ...
            ttk.Label(vis, text="Feature CSV override can be left blank. In analysis+visual mode, v1.4.6 auto-detects human_translation_kht_feature_frame.csv or *_time_resolved_feature_frame.csv from the analysis folder.", wraplength=800).pack(anchor="w", pady=(0,6))
            ...
            fr3 = ttk.LabelFrame(vis, text="Visualizer render mode"); fr3.pack(fill="x", pady=8)
            ttk.Radiobutton(fr3, text="Public/TSC version", value="public_tsc", variable=self.vars["visual_render_mode"]).pack(anchor="w", padx=8, pady=3)
            ttk.Radiobutton(fr3, text="Lab/Diagnostic version", value="lab", variable=self.vars["visual_render_mode"]).pack(anchor="w", padx=8, pady=3)
        ...
        def ns(self):
            ...
            mapping = {
                ...
                "visual_events":"--visual-events", "visual_feature_csv":"--visual-feature-csv", "visual_out":"--visual-out", "visual_render_mode":"--visual-render-mode"
            }
